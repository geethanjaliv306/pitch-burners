...
.btn-info:hover {
  ...
  .myTorunments-table .myTorunments-head .myTorunments-head-col.name, .myTorunments-table
  .myTorunments-head .myTorunments-body-col.name, .myTorunments-table .myTorunments-body 
  .myTorunments-head-col.name, .myTorunments-table .myTorunments-body .myTorunments-body-col.name {
    flex: 0 0 calc(20% - 10px);
}
/*.myTorunments-table .myTorunments-body .myTorunments-body-col {
    max-width: unset !important;
    flex: unset !important;
}

.myTorunments-table .myTorunments-head .myTorunments-head-col{
    max-width: unset !important;
    flex: unset !important;
}
  .myTorunments-body{
        ...
        <div class="row h-100">
            <div class="col-12">
               <div class="title-wrap h-100">
                    <h2 ><a class="my-torunments-back" href="{{ route('total-teams') }}"></a>Not Applied Teams</h2>
                   <div class="d-flex">
                    <a class="btn btn-yellow me-2" href="{{ route('notapplied_csv')}}"><i class="fa-solid fa-download me-2"></i>Download Teams</a>
                    <a class="btn btn-yellow" href="{{ route('notappliedPlayers_csv')}}"><i class="fa-solid fa-download me-2"></i>Download Players</a>
                </div>
              </div>
            </div>
        </div>
...
<section class="my-torunments-wrap">
    <div class="container">
        <div class="d-flex" style="gap:32px;">
            <p style="color: #008E9B">Total Teams: <strong style="color: black">{{ $total_team_count }}</strong></p>
            </p>
        </div>
        <form method="GET" action="{{ route('not-applied-teams') }}" id="searchForm">
            <div class="row mb-3 align-items-center">
                ...
                <div class="col-md-2">
                    <a href="{{ route('total-teams') }}" class="btn btn-warning m-0" style="height: 40px;">View All Teams</a>
                </div>
                @if($search)
                <div class="col-md-2 mt-2">
                    <a href="{{ route('not-applied-teams') }}" class="btn btn-secondary w-100" id="resetButton">Reset</a>
                </div>
                @endif
            </div>
        </form>
        <div class="myTorunments-table">
            <div class="myTorunments-head">
                ...
                <div class="myTorunments-head-col name has-myTorunments-myTeam">Registered At</div>
            ...
            @else
                @foreach($notAppliedTeams as $index => $team)
                    <div class="myTorunments-body">
                        <div class="myTorunments-body-col sno has-myTorunments-myTeam">{{ $notAppliedTeams->firstItem() + $index }}</div>
