...
class TeamsController extends Controller
{
   ...
   public function Teams(){
    ...
    public function getPlayingElevens($teamId, $scheduleMatchId)
{
    Log::info('Team ID passed to route:', ['teamId' => $teamId]);
    ...
    if (!$scheduleMatch) {
        return response()->json(['error' => 'Schedule match not found'], 400);
    }

    if (!in_array($teamId, [$scheduleMatch->team1, $scheduleMatch->team2])) {
        return response()->json(['error' => 'Team not part of this match'], 400);
    }

    $matchType = $scheduleMatch->type;

    $players = Player::where('team_id', $teamId)
        ->where(function ($query) use ($matchType) {
            $query->where('ball_preferences', $matchType)
                  ->orWhere('ball_preferences', 'All');
        })
        ->get();

    $team = Team::find($teamId);

    return response()->json(['players' => $players, 'teamId' => $teamId, 'teamLogo' => $team->logo ?? null, 'teamName' => $team->name ?? null, 'scheduleMatchId'=> $scheduleMatchId, 'matchType' => $matchType]);
}
