 ...
 public function getInningsDetails($id, $isSecondInnings , $isSuperOver , $isSecondInningsSuperOver, $isSecondSuperOver, $isSecondSuperOverSecondInnings){
    ...
    public function getSquadsPlayers(Request $request) {
        ...
        if($match_id) {
            $squadA_data = Team1Detail::where('team1_details.match_id', $match_id->id)->where('team1_details.team_id', $squadA_id)
                        ...
                        ->select( 'players.name as name', 'players.image as imageUrl', 'players.role as player_role', 'team1_details.player_id', 'team1_details.captain', 'team1_details.wicketkeeper', 'team1_details.12th_man',)
                        ->get();
            $squadB_data = Team2Detail::where('team2_details.match_id', $match_id->id)->where('team2_details.team_id', $squadB_id)
                        ...
                        ->select( 'players.name as name', 'players.image as imageUrl', 'players.role as player_role', 'team2_details.player_id', 'team2_details.captain', 'team2_details.wicketkeeper', 'team2_details.12th_man',)
                        ->get();
            ...
            foreach($squadA_data as $data){
                if($data->captain && $data->wicketkeeper){
                    ...
                    $data['role'] = ucfirst(strtolower($data->player_role));
                } else {
                    $data['role'] = 'Player';
                }
            }
    ...
    public function createOrUpdateMatchPlayers(Request $request) {
        ...
        if(isset($match_players)) {
            ...
            $diff_old_batsmen->each(function (int $item) use ($match_id, $team_id, $score_board_ctrl){
                $not_exist_in_bbb = BallByBall::where('match_id', $match_id)
                ->where('batting_team_id', $team_id)->where('striker_id', $item)
                ->orWhere('non_striker_id', $item)->orWhere('dismissed_batsmen', $item)->doesntExist();

                if($not_exist_in_bbb) {
                    $score_board_ctrl->deleteBatsman($item);
                }
            });
    ...
    public function getTeamScore($match_id) {
        ...
        'batting_score' => "Yet To Bat",
        'bowling_score' => "Yet To Bat",
        'is_data_there' => $is_data_there,
        'curr_over' => "0.0/0",
    ...
    public function changematchstatus(Request $request , $matchId) {
        try {
            ...
            $fullDetails = $winningTeam . ' ' . $details;
