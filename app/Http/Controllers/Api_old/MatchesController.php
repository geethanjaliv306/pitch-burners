    ...
    public function storeMatchDetails(Request $request)
{
  ...
  public function MatchFixtures(){
        ...
        ->where('schedule_matches.deleted_at', '=', NULL)
        ...
        foreach($matchFixtures as $fixture) {
            ...
            if(isset($match)) {
                ...
                if(strtolower($status) == 'completed') {
                    ...
                    if($match_score->team_id == $fixture->team2){
                        $fixture->is_teamA_won = false;
        ...
        //return response()->json(['matchFixtures' => [] , 'getSeasons' => [] ,'Venues' =>  [], 'Teams' => [], 'live' => [], 'upcoming' => [], 'completed' => [], 'cancelled' => []]);
  }
 public function webMatchFixtures($params = []){
   
   		if( !isset($params['hide_match']) || $params['hide_match'] != 0) {
   			return response()->json(['matchFixtures' => [] , 'getSeasons' => [] ,'Venues' =>  [], 'Teams' => [], 'live' => [], 'upcoming' => [], 'completed' => [], 'cancelled' => []]);
        }
  ...
  public function getInningsDetails($id, $isSecondInnings , $isSuperOver , $isSecondInningsSuperOver, $isSecondSuperOver, $isSecondSuperOverSecondInnings){
    ...
    public function getTeamScore($match_id) {
        ...
        if(isset($match_data) && isset($is_data_there)) {
            ...
            //NOTE: For Innings change
            $new_inning = $current_inning + 1;
            $is_new_inning_started = BallByBall::where('match_id', $match_id)->where('innings_completed', $new_inning)
            ->where('over_number', -1)->orderBy('id', 'desc')->first();
            if(isset($is_new_inning_started)){
                $batting_team_id = $is_data_there->bowling_team_id;
                $bowling_team_id = $is_data_there->batting_team_id;
            ...
            if(isset($bowling_team_initial)) {
                ...
                $int_part = (int) $str_batting_cur_overs[0]; $float_part = (int) isset($str_batting_cur_overs[1]) ? $str_batting_cur_overs[1]: '0';

                $remaining_balls = ((int) $match_total_overs * 6) - (($int_part * 6) + $float_part);
