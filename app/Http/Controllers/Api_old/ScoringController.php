...
class ScoringController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            'total_runs' => $validatedData['total_runs'],
            ...
            'total_score' =>  $validatedData['totalscore'] ?? $newTotalScore ,
        ...
        if ($validatedData['batter_is_out'] == 1) {
            ...
            if (isset($validatedData['Dismissedplayeroutype']) &&
                ...
                in_array($validatedData['Dismissedplayeroutype']['StrikerandNonstriker']['dismissaltype'],
                    ['Bowled', 'Caught', 'Caught Behind', 'Caught & Bowled', 'Stumped', 'Hit Wicket'])
            ) {
            ...
            // Run Out scenarios
            elseif (($validatedData['batter_is_out'] == 1 &&
                    ($validatedData['dismissal_type'] === 'Run Out'
                    //  || $validatedData['dismissal_type'] === 'Run Out (Mankaded)'
                     )
                    )
            ) {
                Log::info('herecoming4');
            ...
            elseif (($validatedData['batter_is_out'] == 1 && $validatedData['dismissal_type'] === 'RetiredOut')) {
        ...
        elseif (isset($isNoBallOrWide) && $isNoBallOrWide && !$validatedData['batsmanHitBall']) {
            Log::info('herecoming6');
            // No ball or wide where batsman didn't hit the ball
            if ($previousBallData) {
                $BatterData = array_merge($BatterData, [
                    'match_id' => $validatedData['match_id'],
                    'team_id' => $validatedData['batting_team_id'],
        ...
        // Normal ball scenarios
            ...
            if(isset($validatedData['dismissal_type']) && ($validatedData['batter_is_out'] == 1)){
                $dismissalType = $validatedData['dismissal_type'];
            }
            Log::info('herecoming7');
            // Normal case for valid balls
            $BatterData = array_merge($BatterData, [
        ...
        DB::commit();
        // $matches_ctrl = new MatchesController();
        // $data = $matches_ctrl->getPresentMatchData($validatedData['match_id']);
        // PodcastProcessed::dispatch($data);


        // return response()->json(['message' => 'Ball data saved successfully', 'data' => [$ball, $batter, $bowler, $fielding, $commentary]], 201);
        ...
        public function getBowlerStats( $matchId, $bowlerId){
            ...
            try {
                ...
                // Fetch the last row for the bowler stats based on the match ID and player ID
    ...
    public function deleteLastBall($matchId)
        {
            ...
            try {
                ...
                if($lastBall->over_number == -1) {
                    ...
                    DB::commit();
                    return response()->json(['message' => 'Inning last ball data deleted successfully' , 'synced' => '✔ Undo done', 'lastBall' => $lastBall, 'inning' => $lastball_inning, 'is_inning_changed' => true], 200);
                    exit();
                }
                ...
                if(isset($is_last_ball_inning_data_exist)){
                    ...
                    foreach($batter_ids as $id) {
                        $batter_id = $id;
                        $is_player_data_exist_in_stats = PlayerBattingStats::where('match_id', $matchId)
                        ->where('team_id', $last_ball_batting_team)->where('player_id', $id)
                        ->orderBy('id', 'desc')
                        ->select(
                        ...
                        ->first();
                        if(!isset($is_player_data_exist_in_stats)) {
               ...
               //return response()->json(['message' => 'Last ball data deleted successfully' ,'lastBall' =>$lastBall, 'batter_ids', $batter_ids, 'unwanted_players' => $scoreboard_unwanted_players, 'bowlers' => $upto_last_ball_bowlers, 'unwanted_bowlers' => $unwanted_bowlers], 200);
               return response()->json(['message' => 'Last ball data deleted successfully' , 'synced' => '✔ Undo done', 'lastBall' =>$lastBall], 200);
            } catch (\Exception $e) {
                DB::rollBack();
    ...
    public function updatePoints(Request $request) {
        ...
        public function fetchBallByBallData($matchId){
            ...
            $ballData = BallByBall::where('match_id', $matchId)->get();

            if ($ballData->isEmpty()) {
                return response()->json([
                    'message' => 'No ball-by-ball data found for this match.',
                ], 404);
            }

            return response()->json([
                'message' => 'Ball-by-ball data fetched successfully.',
                'data' => $ballData
            ], 200);
        }
