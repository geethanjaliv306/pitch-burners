  ...
  public function MatchFixtures(){
        ...
        foreach($matchFixtures as $fixture) {
            ...
            if(isset($match)) {
                ...
                $result = new stdClass();
                $result->winner_name = null;
                $result->runner_name = null;
                $result->run_diff = null;
                $result->wicket_diff = null;
                $result->diff_text = null;
                if(strtolower($status) == 'completed' && $team1_score != $team2_score) {
                    $run_diff = (int)$team1_score - (int)$team2_score;
                    $winner = $team1->name; $runner = $team2->name;
                    $wicket_diff = null;
                    $diff_text = 'runs';
                    if($team1_score < $team2_score){
                        $winner = $team2->name;
                        $runner = $team1->name;
                        // $run_diff = $team1_score - $team2_score;
                        $wicketed = BallByBall::where('match_id', $match->id)->where('batting_team_id', $team2->id)->where('is_wicket', 1)->count();
                        // $not_wicketed = Team2Detail::where('match_id', $match->id)->where('team_id', $team2->id)->pluck('player_id')->count();
                        $wicket_diff = 10 - $wicketed;
                        $run_diff = null;
                        $diff_text = 'wickets';
                    }
                    ...
                    $result->wicket_diff = $wicket_diff;
                    $result->diff_text = $diff_text;
                    // $result->text = "$winner won by $run_diff";
                    $fixture->result = $result;
                }
                if((strtolower($status) == 'active' && $team1_score == $team2_score) || (strtolower($status) == 'completed' && $team1_score == $team2_score)) {
        ...
        uasort($live, function($a, $b) {
            $a_date = Carbon::createFromDate($a->match_date_time);
            $b_date = Carbon::createFromDate($b->match_date_time);
  ...
  public function getInningsDetails($id, $isSecondInnings , $isSuperOver , $isSecondInningsSuperOver, $isSecondSuperOver, $isSecondSuperOverSecondInnings){
    ...
    public function getTeamScore($match_id) {
        ...
        if(isset($match_data) && isset($is_data_there)) {
            ...
            $batting_team_wickets = BallByBall::where('match_id', $match_id)->where('batting_team_id', $batting_team_id)->where('is_wicket', 1)->get()->count();
            $batting_team_initial = BallByBall::where('match_id', $match_id)->where('batting_team_id', $batting_team_id)->orderBy('id', 'ASC')->first();
            $batting_team_final = BallByBall::where('match_id', $match_id)->where('batting_team_id', $batting_team_id)->orderBy('id', 'DESC')->first();
            $batting_score = "0/0";
            $batting_overs = "0.0/0";
            if(isset($batting_team_initial)) {
                ...
                $total = isset($batting_team_data) ? $batting_team_data->total_score : 0;
                $batting_score = "$total/$batting_team_wickets";
				$batting_overs = isset($batting_team_data) ? "$batting_team_data->total_overs/$match_data->overs" : "0.0/0";
            }
            $current_bowling_innings = $current_inning > 0 ? $current_inning - 1 : $current_inning;
            $bowling_team_wickets = BallByBall::where('match_id', $match_id)->where('batting_team_id', $bowling_team_id)->where('is_wicket', 1)->where('innings_completed', $current_bowling_innings)->get()->count();
            $bowling_team_initial = BallByBall::where('match_id', $match_id)->where('batting_team_id', $bowling_team_id)->orderBy('id', 'ASC')->where('innings_completed', $current_bowling_innings)->first();
            $bowling_team_final = BallByBall::where('match_id', $match_id)->where('batting_team_id', $bowling_team_id)->orderBy('id', 'DESC')->where('innings_completed', $current_bowling_innings)->first();
            $bowling_score = "0/0";
            $bowling_overs = "0.0/0";
            if(isset($bowling_team_initial)) {
                ...
                $total = isset($bowling_team_data) ? $bowling_team_data->total_score : 0;
                $bowling_score = "$total/$bowling_team_wickets";
                $bowling_overs = isset($bowling_team_data) ? "$bowling_team_data->total_overs/$match_data->overs" : "0.0/0";
            }
            // $curr_over_number = $is_data_there->over_number;
            // $over = $is_data_there->valid_ball_count;
            // if($over == 6) {
            //     // $curr_over_number = ($match_data->overs + 1) == $curr_over_number ? ($curr_over_number - 1) : ($curr_over_number + 1);
            //     $curr_over_number  = ($match_data->overs - 1);
            //     $over = ($curr_over_number) . "/" . $match_data->overs;
            // }else{
            //     if($curr_over_number == 0){
            //         $over = "0.0/".$match_data->overs;
            //     }else{
            //         $over = $curr_over_number + ($over / 10);
            //         if($over != 1)
            //         $over = ($over) ."/" . $match_data->overs;
            //     }
            // }
            // $valid_ball_count = $is_data_there->valid_ball_count;
            // if($valid_ball_count == 6) {
            //     $over_number = $is_data_there->over_number + 1;
            //     $over = "$over_number.0/$match_data->overs";
            // }else {
            ...
            'batting_final' => $batting_team_final,
            'bowling_final' => $bowling_team_final,
            'curr_over' => $over,
    ...
    public function scroeBoardPresentTeamDetails($match_id) {
        ...
        if(isset($is_stats_exist)) {
            $match_data = MatchGame::where('id', $match_id)->orderBy('id', 'ASC')->first();
            $stats->overs = $match_data->overs;
            ...
            $stats->name = $batting_team_data->name;
            $stats->curr_over = "$batting_team_data->totalOvers/$match_data->overs";
